...
;;;Abstract
;;;
;;;	This library is  like (vicare language-extensions simple-match),
...
(define (build-pattern-matching-code pattern-stx in-expr-stx
  ...
  (with-syntax (((expr)        (generate-temporaries #'(#f)))
		(IN-EXPR       in-expr-stx))
    (syntax-case pattern-stx (let let/tags quote quasiquote and or not apply eval syntax)
...
(define (vector-with-ellipsis-as-tail? stx)
  ...
  (syntax-case stx (let)
    (#(?item0 ?item ... (let ?var ?ellipsis))
     (ellipsis? #'?ellipsis))
    (#(?item0 ?item ... (let/tags ?var ?ellipsis))
     (ellipsis? #'?ellipsis))
    (#(?item0 ?item ... ?ellipsis)
     (ellipsis? #'?ellipsis))
    (_
