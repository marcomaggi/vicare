...
(define-with-caller (parse-let-bindings (bindings-stx top-id synner)
  ...
  ;;
  ;;where each VAR is an identifier to be used to create a binding, each
  ;;TAG is the identifier of the type tag and each SYNTAX-BINDING is the
  ;;associated LET-SYNTAX binding.  If a variable is tagged with TOP-ID:
  ...
  ;;         ?body0 ?body ...))
  ;;
  ;;TOP-ID must the the identifier bound to the "<top>" tag; this tag is
  ;;used as default when no tag  is given for a binding.  SYNNER must be
  ;;a closure to be used to raise syntax violations.
  ;;
...
(define-with-caller (parse-let-values-bindings (bindings-stx top-id synner)
					       ((values-vars		'())
						(syntax-bindings	'())))
  ;;Recursive function.  Parse  the syntax object BINDINGS-STX expecting
  ...
  ;;bindings.  If a variable is tagged with TOP-ID: no syntax binding is
  ;;generated.
  ;;
...
(define (<parsed-spec>-concrete-fields-names spec)
  ...
  ;;
  (map <field-spec>-name-id
    (<parsed-spec>-concrete-fields spec)))

;;; --------------------------------------------------------------------

(define (<label-spec>-satisfaction-clauses S)
  (let-values (((virtual-mutable-fields virtual-immutable-fields)
		(%field-spec-satisfaction-clauses (<parsed-spec>-virtual-fields  S))))
    (list (list (<parsed-spec>-name-id S)
		(<parsed-spec>-public-constructor-id S)
		(<parsed-spec>-public-predicate-id S))
	  (list #'aux.parent		(<parsed-spec>-parent-id S))
	  (cons #'aux.virtual-fields	virtual-mutable-fields)
	  (cons #'aux.virtual-fields	virtual-immutable-fields)
	  (cons #'aux.methods		(<parsed-spec>-methods-table S))
	  (list #'aux.getter		(<parsed-spec>-getter S))
	  (list #'aux.setter		(<parsed-spec>-setter S))
	  (list #'aux.nongenerative	(<parsed-spec>-nongenerative-uid S))
	  (list #'aux.shadows		(<parsed-spec>-shadowed-identifier S))
	  )))

(define (<class-spec>-satisfaction-clauses S)
  (let-values (((concrete-mutable-fields concrete-immutable-fields)
		(%field-spec-satisfaction-clauses (<parsed-spec>-concrete-fields S)))
	       ((virtual-mutable-fields virtual-immutable-fields)
		(%field-spec-satisfaction-clauses (<parsed-spec>-virtual-fields  S))))
    (list (list (<parsed-spec>-name-id S)
		(<parsed-spec>-public-constructor-id S)
		(<parsed-spec>-public-predicate-id S)
		(<class-spec>-record-type-id S))
	  (list #'aux.parent		(<parsed-spec>-parent-id S))
	  ...
	  (list #'aux.setter		(<parsed-spec>-setter S))
	  (list #'aux.nongenerative	(<parsed-spec>-nongenerative-uid S))
	  (list #'aux.sealed		(<parsed-spec>-sealed? S))
	  (list #'aux.opaque		(<parsed-spec>-opaque? S))
	  (list #'aux.abstract		(<parsed-spec>-abstract? S))
	  )))
...
(define (%field-spec-satisfaction-clauses fields)
  (receive (mutables immutables)
    ...
    (values (map (lambda (F)
		   (let ((name     (<field-spec>-name-id     F))
			 (tag      (<field-spec>-tag-id      F))
			 (accessor (<field-spec>-accessor-id F))
			 (mutator  (<field-spec>-mutator-id  F)))
...
(define (<parsed-spec>-accessor-transformer spec)
  ...
  (with-syntax
	...
	;;   #`(#,var-id . #,args-stx)
	;;
	;;which has  been previously decomposed in  VAR-ID and ARGS-STX;
	;;VAR-ID is a tagged variable whose actual value is in EXPR-STX.
	;;
