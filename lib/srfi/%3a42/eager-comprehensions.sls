     ...
     (:do cc
          (let ((b arg2))
            (if (not (and (integer? b) (exact? b)))
                (error ':range
		  "arguments of :range are not exact integer (use :real-range?)" 0 b 1)))
          ((var 0))
          (< var b)
          (let ())
          #t
      ...
      (if g1
          (if g2
              (if (null? args)
                  ...
                  (error 'dispatch-unoin "dispatching conflict"
			 args (d1 '()) (d2 '())))
              g1)
          (if g2 g2 #f)))))
         ...
         (do-ec qualifier
                (if (< i len)
                    (begin (vector-set! vec i expression)
                           (set! i (+ i 1)) )
                    (error 'vector-of-length-ec
		      "vector is too short for the comprehension")))
         (if (= i len)
             vec
	   (error 'vector-of-length-ec
	     "vector is too long for the comprehension")))))))
