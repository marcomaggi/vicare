...
;;
;;   (set! tm:leap-second-table (tm:read-tai-utc-date "tai-utc.dat"))
;;
	 ...
	 ((_ ?type)
	  (define-argument-validation (?type who obj)
	    ...
	    (assertion-violation who
	      ...
	      obj))))))
  (define-type-type-validation time-tai)
  (define-type-type-validation time-utc)
  (define-type-type-validation time-monotonic)
  (define-type-type-validation time-thread)
  (define-type-type-validation time-process)
  (define-type-type-validation time-duration))
...
(module (time-difference
  ...
  (define (add-duration time duration)
	...
	((time		time)
	 (time-duration	duration))
      ($add-duration time duration (make-time ($time-type time) #f #f))))
  ...
  (define (subtract-duration time duration)
	...
	((time		time)
	 (time-duration	duration))
      ($subtract-duration time duration (make-time ($time-type time) #f #f))))
...
;;; --------------------------------------------------------------------
  ...
  (define ($time-difference time1 time2 time3)
  ...
  (define ($add-duration time1 duration time3)
  ...
  (define ($subtract-duration time1 duration time3)
...
(define time-tai->date
  (case-lambda
   ((time)
    (time-tai->date time (tm:local-tz-offset)))
   ((time tz-offset)
	...
	((time-tai	time))
	  ...
	  (let* ((time^ (let ((T (time-tai->time-utc time)))
			  ($subtract-duration T (make-time time-duration 0 1) T)))
		 (date ($time->date time^ tz-offset time-utc)))
	    ($set-date-second! date 60)
	    date)
	($time->date ($time-tai->time-utc time) tz-offset))))))

(define time-utc->date
  (case-lambda
   ((time)
    (time-utc->date time (tm:local-tz-offset)))
   ((time tz-offset)
    (define who 'time-utc->date)
    (with-arguments-validation (who)
	((time-utc	time))
      ($time->date time tz-offset)))))
...
(define time-monotonic->date
  ...
  (case-lambda
   ((time)
    (time-monotonic->date time (tm:local-tz-offset)))
   ((time tz-offset)
...
(define ($time->date time tz-offset)
