  ...
  (let-values
	...
	(let ((status (px.waitpid child-pid 0)))
	  (if (and (px.WIFEXITED status)
	      ...
	      ;;Read the output from "sendmail" generated by the "-v" flag.  Sendmail
	      ;;writes to its stdout, rather than to its stderr.
	      (let ((logmsg (%read-stdout-from-child child-stdout->parent)))
		logmsg)
	    (error __who__
