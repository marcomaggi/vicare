...
;;;Contents: download files using curl
  ...
  (posix::fork-with-binary-ports
    ...
    (lambda (child-pid stdin-port stdout-port stderr-port)
	  ...
	  (let ((status (posix::waitpid child-pid 0)))
		...
		(let ((out (get-bytevector-all stdout-port))
	      ...
	      (error __who__ "curl process exited abnormally" status)))
    ...
    (lambda ()
      (guard (E (else
		 (print-condition E)
		 (exit 1)))
	(posix::execvp "curl" `("curl" "--silent" "--show-error" ,@opts ,url))))))
