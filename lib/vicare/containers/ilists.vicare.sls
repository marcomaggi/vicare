...

#!vicare
(library (vicare containers ilists)
  (export
    iq
  ...
  (syntax-rules ()
    ((:optional rest default)
     (cond
       ...
       (else (error #f "Too many arguments"))))))
...
(define (make-ilist len . maybe-elt)
  ...
  (let ((elt (cond ((null? maybe-elt) #f) ; Default value
		   ...
		   (else (error #f "Too many arguments to MAKE-ILIST"
				(ipair len maybe-elt))))))
    (do ((i len (- i 1))
	 (ans '() (ipair elt ans)))
	((<= i 0) ans))))
...
(define (iiota count . maybe-start+step)
  ...
  (if (< count 0) (error #f "Negative step count" iiota count))
