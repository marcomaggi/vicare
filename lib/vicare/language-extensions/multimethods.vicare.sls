...

#!vicare
(library (vicare language-extensions multimethods (0 4 2016 5 24))
...
(define (generic-function-implementation who methods-alist-func cache-store cache-ref
  ...
  (define/typed {applicable-methods <list>}
	...
	(let ((methods (mt::compute-applicable-methods signature methods-alist-func)))
	  ...
	  methods)))
  (define/typed {method-called? <boolean>} #f)
  (define/typed (is-a-next-method-available?)
    (null? applicable-methods))
  (define/typed (call-methods)
...
(define (generic*-function-implementation
  ...
  (define/typed {primary-method-called? <boolean>} #f)
  (define/typed {reject-recursive-calls? <boolean>} #f)
  (define/typed (is-a-next-method-available?)
...
(define-syntax (define-method stx)
  ...
  ;;generic function.
  ;;
  (define/typed who 'define-method)
  (define/typed (main generic-function-spec table-key formals-stx body-stx)
  ...
  (syntax-case stx ()
    ((_ ?generic-function ?keyword (?type-id ...) ?closure)
     ...
     #`(?generic-function #,(case-identifiers #'?keyword
			  ...
			  (list (type::type-unique-identifiers ?type-id) ...) ;this is the signature
			  ?closure))
...

(define (type-unique-identifiers-of obj)
  ;;Return the list of UIDs in the class hierarchy of OBJ.  The order of the tests is
