...

#!r6rs
(library (vicare language-extensions increments)
  (export
...
(define-syntax ($pre-incr! stx)
  (syntax-case stx ()
    ((_ ?id)
     (identifier? #'?id)
     #'(begin
	 (set! ?id ($add-number-fixnum ?id 1))
	 ?id))
    ((_ ?id ?step)
     (identifier? #'?id)
     #'(begin
	 (set! ?id ($add-number-number ?id ?step))
	 ?id))
    ((_ ?expr)
...
(define-syntax ($pre-decr! stx)
  (syntax-case stx ()
    ((_ ?id)
     (identifier? #'?id)
     #'(begin
	 (set! ?id ($sub-number-fixnum ?id 1))
	 ?id))
    ((_ ?id ?step)
     (identifier? #'?id)
     #'(begin
	 (set! ?id ($sub-number-number ?id ?step))
	 ?id))
    ((_ ?expr)
     #'($add-number-fixnum ?expr 1))
    ((_ ?expr ?step)
