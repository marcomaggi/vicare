...
;;;-------------
;;;
;;;   The collection of methods in a generic function is an alist; each entry has the
;;;   format:
;;;
;;;      (signature . closure)
;;;
...

#!vicare
(library (vicare language-extensions multimethods methods-table (0 4 2016 5 24))
  ...
  (export
    ...
    ;;Commented out because not  used by outer code; But it can  be exported if there
    ;;is the need.  (Marco Maggi; Mon Nov 25, 2013)
    ;;
    #;add-method-to-methods-alist)
  (import (vicare (0 4))
    (only (vicare language-extensions multimethods symbols-tree (0 4))
	  tree-cons
	  treeq))
...
(define-syntax define-methods-table
  ;;Used in the body of DEFINE-GENERIC to define a methods table with its cache.
  ;;
  (syntax-rules ()
    ;;We could automatically generate most of  the identifier arguments, we do not do
    ;;it for hygiene sake.
    ;;
    ((_ ?generic-function ?number-of-arguments
	?table-name-func ?table-add ?cache-name ?cache-store ?cache-ref ?init)
     (begin
       (define/typed ?table-name-func
	 ;;We need to access the table through  a function; this is because the table
	 ;;must be mutated by code  across different libraries when generic functions
	 ;;are merged.  It is not possible for  code of a library to mutate a binding
	 ;;in another library.
	 ;;
	 (let ((table ?init))
	   (case-lambda
	    ...
	    ((new-table)
	     (set! table new-table)))))
       (define/typed {?cache-name <list>} '()) ;symbols tree
       (define/typed (?cache-store signature methods)
	 (set! ?cache-name (tree-cons (map car signature) methods ?cache-name)))
       (define/typed (?cache-ref signature)
	 (treeq (map car signature) ?cache-name #f))
       (define/typed (?table-add signature closure)
...
(define (add-method-to-methods-alist methods-alist-func signature closure)
  ;;Add a method's entry to the alist of methods; return the modified method alist.
  ;;
  ;;A new method entry is added only if no method with the selected signature already
  ;;exists.   If  a  method  with  the  signature  already  exists,  its  closure  is
...
(define (compute-applicable-methods call-signature methods-alist-func)
  ;;Filter  out  from  the  alist  returned by  METHODS-ALIST-FUNC  the  methods  not
  ;;applicable to a tuple of arguments  with types in the tuple CALL-SIGNATURE.  Then
  ;;sort the list of applicable methods so that the more specific are the first ones.
...
(define (applicable-method-signature? call-signature method-signature)
  ;;Return  true if  a method  with METHOD-SIGNATURE  can be  applied to  a tuple  of
...
(define (more-specific-signature? signature1 signature2 call-signature)
  ...
  ;;CALL-SIGNATURE.   This  function must  be  applied  to applicable  methods.   The
  ;;longest signature is more specific, by definition.
  ;;
  (let next-argument-type ((signature1     signature1)
	...
	;;If we are here:  the two signatures have EQ?  car values;  this is an error
	;;because ADD-METHOD-TO-METHODS-ALIST should have  detected this and replaced
	;;one method's closure with the other in the alist of methods.
