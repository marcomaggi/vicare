...

(if-building-rotation-boot-image? "system-value API definition"
    ;;While building a new boot image: these  bindings are the ones from the old boot
    ;;image.
    (import (only (vicare) system-value system-value-gensym))
  ;;During normal execution: these bindings are generated here.
  (begin
    ...
    (define* (system-value {x symbol?})
      ...
      ;;function  (CONS, CAR,  ...)   one of  the  operations is  to  put the  actual
      ;;function (a closure  object) in the "value"  field of a gensym,  and then put
      ;;such  gensym in  the  property list  of  the  symbol being  the  name of  the
      ;;primitive, using an internal gensym (bound to SYSTEM-VALUE-GENSYM) as key.
      ;;
    ...
    #| end of BEGIN |# )
  #| end of IF-BUILDING-ROTATION-BOOT-IMAGE? |# )
