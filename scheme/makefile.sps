...
;;; POSIX functions
    ...
    (S_ISSOCK					i v posix)
    (WCOREDUMP					i v posix)
    (WEXITSTATUS				i v posix)
    (WIFEXITED					i v posix)
    (WIFSIGNALED				i v posix)
    (WIFSTOPPED					i v posix)
    (WSTOPSIG					i v posix)
    (WTERMSIG					i v posix)
    (accept					i v posix)
    (access					i v posix)
    ...
    (connect					i v posix)
    (ctermid					i v posix)
    (ctermid/string				i v posix)
    (dup					i v posix)
    (dup2					i v posix)
    (environ					i v posix)
    (environ->table				i v posix)
    (environ-table				i v posix)
    (errno->string				i v posix)
    (execl					i v posix)
    (execle					i v posix)
    (execlp					i v posix)
    (execv					i v posix)
    ...
    (getpwnam					i v posix)
    (getpwuid					i v posix)
    (getservbyname				i v posix)
    (getservbyport				i v posix)
    (getsid					i v posix)
    (getsockname				i v posix)
    (getsockopt					i v posix)
    (getsockopt/int				i v posix)
    (getsockopt/linger				i v posix)
    ...
    (h_strerror					i v posix)
    (host-entries				i v posix)
    (inet-aton					i v posix)
    (inet-ntoa					i v posix)
    (inet-ntoa/string				i v posix)
    (inet-ntop					i v posix)
    (inet-ntop/string				i v posix)
    (inet-pton					i v posix)
    (interprocess-signal->string		i v posix)
    (ioctl					i v posix)
    (kill					i v posix)
    (link					i v posix)
    (listen					i v posix)
    ...
    (readdir					i v posix)
    (readdir/string				i v posix)
    (readlink					i v posix)
    (readlink/string				i v posix)
    (readv					i v posix)
    (realpath					i v posix)
    (realpath/string				i v posix)
    (recv					i v posix)
    (recvfrom					i v posix)
    (rename					i v posix)
    (rewinddir					i v posix)
    ...
    (stat					i v posix)
    (strftime					i v posix)
    (strftime/string				i v posix)
    (symlink					i v posix)
    (system					i v posix)
    (table->environ				i v posix)
    (tcgetpgrp					i v posix)
    (tcgetsid					i v posix)
    (tcsetpgrp					i v posix)
    (telldir					i v posix)
...
;;;
    ...
    (struct-stat-st_ino				i v posix)
    (struct-stat-st_dev				i v posix)
    (struct-stat-st_nlink			i v posix)
    (struct-stat-st_uid				i v posix)
    (struct-stat-st_gid				i v posix)
    (struct-stat-st_size			i v posix)
    (struct-stat-st_atime			i v posix)
    (struct-stat-st_atime_usec			i v posix)
    (struct-stat-st_mtime			i v posix)
    (struct-stat-st_mtime_usec			i v posix)
    (struct-stat-st_ctime			i v posix)
...
;;;
    ...
    (file-is-regular-file?			i v posix)
    (file-is-symbolic-link?			i v posix)
    (file-is-socket?				i v posix)
    (file-is-fifo?				i v posix)
...
;;;
    ...
    (struct-hostent-h_aliases			i v posix)
    (struct-hostent-h_addrtype			i v posix)
...
;;;
    ...
    (memcpy					$for)
;;;
    (pointer-c-ref-uint8			$for)
    (pointer-c-ref-sint8			$for)
    (pointer-c-ref-uint16			$for)
    (pointer-c-ref-sint16			$for)
    (pointer-c-ref-uint32			$for)
    (pointer-c-ref-sint32			$for)
