...
(define-constant VICARE-CORE-BUILT-IN-SYNTAXES-SYNTACTIC-BINDING-DESCRIPTORS
  '( ;;;Core macros that can appear only in a body.
    ...
    (case-define/std				(case-define/std))
    (case-define/typed				(case-define/typed))
    (case-define/checked			(case-define/checked))
...
;;;Core macros that can appear only in an expression.
    ...
    (signature-super-and-sub?			(core-macro . signature-super-and-sub?))
...
;;;Non-core macros.
    ...
    (custom-predicate				(macro . custom-predicate))
    ...
    (hash-function				(macro . hash-function))
    (equality-predicate				(macro . equality-predicate))
    (comparison-procedure			(macro . comparison-procedure))
    (method					(macro . method))
...
(define-constant IDENTIFIER->LIBRARY-MAP
  ...
  '((import					v $language)
    ...
    (type-of					v $language)
    (type-super-and-sub?			v $language)
    (signature-super-and-sub?			v $language)
...
;;;
    ...
    (object-type-spec.equality-predicate-id		$expander)
    (object-type-spec.comparison-procedure-id		$expander)
    (object-type-spec.hash-function-id			$expander)
    (object-type-spec.applicable-hash-function-id	$expander)
