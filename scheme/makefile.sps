...
;;; built-in Scheme object types
    ...
    ;;scheme-type descriptors predicates
    ;;
    ;;NOTE These are commented out because I am  not sure that it makes sense to have
    ;;them.  (Marco Maggi; Fri Jun 3, 2016)
    ;;
    ;; (<bignum>-type-descriptor?)
    ;; (<binary-input-port>-type-descriptor?)
    ;; (<binary-input/output-port>-type-descriptor?)
...
;;; object type descriptors
    ...
    (<pair-type-descr>-rtd			v $language)
    (<pair-type-descr>-rcd			v $language)
    (make-pair-type-descr			v $language)
    (pair-type-descr?				v $language)
    (pair-type-descr.car-des			v $language)
    (pair-type-descr.cdr-des			v $language)

    (<pair-of-type-descr>-rtd			v $language)
    (<pair-of-type-descr>-rcd			v $language)
    (make-pair-of-type-descr			v $language)
    (pair-of-type-descr?			v $language)
    (pair-of-type-descr.item-des		v $language)

    (<list-type-descr>-rtd			v $language)
    (<list-type-descr>-rcd			v $language)
    (make-list-type-descr			v $language)
    (list-type-descr?				v $language)
    (list-type-descr.item-des*			v $language)

    (<list-of-type-descr>-rtd			v $language)
    (<list-of-type-descr>-rcd			v $language)
    (make-list-of-type-descr			v $language)
    (list-of-type-descr?			v $language)
    (list-of-type-descr.item-des		v $language)

    (<vector-type-descr>-rtd			v $language)
    (<vector-type-descr>-rcd			v $language)
    (make-vector-type-descr			v $language)
    (vector-type-descr?				v $language)
    (vector-type-descr.item-des*		v $language)

    (<vector-of-type-descr>-rtd			v $language)
    (<vector-of-type-descr>-rcd			v $language)
    (make-vector-of-type-descr			v $language)
    (vector-of-type-descr?			v $language)
    (vector-of-type-descr.item-des		v $language)
;;;
    (<union-type-descr>-rtd			v $language)
    (<union-type-descr>-rcd			v $language)
    (make-union-type-descr			v $language)
    (union-type-descr?				v $language)
    (union-type-descr.item-des*			v $language)

    (<intersection-type-descr>-rtd		v $language)
    (<intersection-type-descr>-rcd		v $language)
    (make-intersection-type-descr		v $language)
    (intersection-type-descr?			v $language)
    (intersection-type-descr.item-des*		v $language)

    (<complement-type-descr>-rtd		v $language)
    (<complement-type-descr>-rcd		v $language)
    (make-complement-type-descr			v $language)
...
;;;
    ...
    (<scheme-type-spec>-rtd)
    (<scheme-type-spec>-rcd)
    (<scheme-type-spec>					$expander)
    (make-scheme-type-spec				$expander)
    (scheme-type-spec?					$expander)
    (scheme-type-spec.equality-predicate-id		$expander)
