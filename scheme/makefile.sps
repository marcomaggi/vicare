...
(define identifier->library-map
  ...
  '((import					i v $language)
    ...
    (splice-first-expand			i v $language)
    (type-descriptor				i v $language)
    (is-a?					i v $language)
    (slot-ref					i v $language)
    (slot-set!					i v $language)
    ($slot-ref					i v $language)
    ($slot-set!					i v $language)
    (struct-type-descriptor			i v $language)
    (struct-type-and-struct?			i v $language)
    (struct-type-field-ref			i v $language)
    (struct-type-field-set!			i v $language)
    ($struct-type-field-ref			i v $language)
    ($struct-type-field-set!			i v $language)
    (parameterize				i v $language)
    (parameterise				i v $language)
    (parametrise				i v $language)
    ...
    (random					i v $language)
    (gensym?					i v $language)
    (getprop					i v $language)
    (putprop					i v $language)
    (remprop					i v $language)
    (property-list				i v $language)
    (gensym->unique-string			i v $language)
    (symbol-bound?				i v $language)
    (top-level-value				i v $language)
    (reset-symbol-proc!				i v $language)
...
;;;
    ...
    (define-syntax-parameter			i v $language)
    (syntax-parametrise				i v $language)
    (syntax-parameterize			i v $language)
    (syntax-parameterise			i v $language)
    (syntax-parameter-value			i v $language)
...
;;
    ...
    (zero-exact-integer?			i v $language)
    (negative-exact-integer?			i v $language)
    (positive-exact-integer?			i v $language)
    (non-negative-exact-integer?		i v $language)
    (non-positive-exact-integer?		i v $language)
    (lcm					i v r ba se)
    (length					i v r ba se)
    (list					i v r ba se)
    ...
    (latin1-encoded-bytevector?			i v $language)
    (ascii-encoded-string?			i v $language)
    (latin1-encoded-string?			i v $language)
    (string->ascii				i v $language)
    (string->octets				i v $language)
    (octets-encoded-bytevector?			i v $language)
    (octets-encoded-string?			i v $language)
    (octets->string				i v $language)
    (ascii->string				i v $language)
    (ascii-encoded-bytevector?			i v $language)
    ...
    (normalise-uri-encoding			i v $language)
    (uri-encoded-bytevector?			i v $language)
    (uri-encoded-string?			i v $language)
    (percent-encoded-bytevector?		i v $language)
    (percent-encoded-string?			i v $language)
    (percent-encode				i v $language)
    (percent-decode				i v $language)
    (normalise-percent-encoding			i v $language)
    (symbol->string				i v r ba se)
    (symbol=?					i v r ba)
    (symbol?					i v r ba se)
    (tan					i v r ba se)
...
;;;
    ...
    (bytevector-length?				i v $language)
    (bytevector-index?				i v $language)
    (bytevector-word-size?			i v $language)
    (bytevector-word-count?			i v $language)
    (bytevector-index-for-word?			i v $language)
    (bytevector-index-for-word8?		i v $language)
    (bytevector-index-for-word16?		i v $language)
    (bytevector-index-for-word32?		i v $language)
    (bytevector-index-for-word64?		i v $language)
    (bytevector-start-index-and-count-for-word?		i v $language)
    (bytevector-start-index-and-count-for-word8?	i v $language)
    (bytevector-start-index-and-count-for-word16?	i v $language)
    (bytevector-start-index-and-count-for-word32?	i v $language)
    (bytevector-start-index-and-count-for-word64?	i v $language)
    (list-of-bytevectors?			i v $language)
    ...
    (binary-input-port?				i v $language)
    (textual-input-port?			i v $language)
    (binary-output-port?			i v $language)
    (textual-output-port?			i v $language)
    (binary-input/output-port?			i v $language)
    (textual-input/output-port?			i v $language)
    ...
    (unsafe-record-accessor			i v $language)
    (unsafe-record-mutator			i v $language)
    ...
    (make-synonym-transformer			i v $language)
    (synonym-transformer?			i v $language)
    (synonym-transformer-identifier		i v $language)
    (make-compile-time-value			i v $language)
    (compile-time-value?			i v $language)
    (compile-time-value-object			i v $language)
    (syntactic-binding-putprop			i v $language)
    (syntactic-binding-getprop			i v $language)
    (syntactic-binding-remprop			i v $language)
    (syntactic-binding-property-list		i v $language)
    ...
    (set-symbol-value!				i v $language $boot)
    (eval-core					$boot)
...
;;;
    (set-predicate-procedure-argument-validation! i v $language)
    (set-predicate-return-value-validation!	i v $language)
...
;;; POSIX functions
    ...
    (strerror					i v $language $posix stale-posix)
    (errno->string				$posix stale-posix)
    (getenv					i v $language $posix stale-posix)
    (environ					i v $language $posix stale-posix)
    (mkdir					$posix stale-posix)
    (mkdir/parents				$posix stale-posix)
    (real-pathname				$posix stale-posix)
    (file-pathname?				$posix stale-posix)
    (file-string-pathname?			$posix stale-posix)
    (file-bytevector-pathname?			$posix stale-posix)
    (file-absolute-pathname?			$posix stale-posix)
    (file-relative-pathname?			$posix stale-posix)
    (file-colon-search-path?			$posix stale-posix)
    (file-string-colon-search-path?		$posix stale-posix)
    (file-bytevector-colon-search-path?		$posix stale-posix)
    (file-modification-time			$posix stale-posix)
    (split-pathname-root-and-tail		$posix stale-posix)
    (search-file-in-environment-path		$posix stale-posix)
    (search-file-in-list-path			$posix stale-posix)
    (split-pathname				$posix stale-posix)
    (split-pathname-bytevector			$posix stale-posix)
    (split-pathname-string			$posix stale-posix)
    (split-search-path				$posix stale-posix)
    (split-search-path-bytevector		$posix stale-posix)
    (split-search-path-string			$posix stale-posix)
    (vicare-argv0				i v $language $posix stale-posix)
    (vicare-argv0-string			i v $language $posix stale-posix)
