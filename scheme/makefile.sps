...
(options::print-debug-messages? (equal? "yes" (getenv "DEBUG_MESSAGES")))

(options::writing-boot-image? #t)
...
(define debug-printf
  (if (options::print-verbose-messages?)
      (lambda args
...
;;;Non-core macros.
    ,@(process-non-core-macros
	 ...
	 (method				(macro . method))
	 (virtual-method			(macro . virtual-method))
	 (seal-method				(macro . seal-method))
	 (private				(macro . private))
	 (protected				(macro . protected))
	 (public				(macro . public))
...
(include "built-in-scheme-type-specs.scm"		#t)
...
(define-constant LIBRARY-LEGEND
  ...
  ;;
  ;;abbreviation		name					visible? required?
  `((v				(vicare)				#t	#t)
    (r				(rnrs)					#t	#t)
    (r5				(rnrs r5rs)				#t	#t)
    (ct				(rnrs control)				#t	#t)
...
;;;
    ...
    (system-posix		(vicare system posix)			#t	#t)
;;;
    ($compiler			(vicare compiler)			#t	#t)
    ($expander			(vicare expander)			#t	#t)
    ($programs			(vicare programs)			#t	#t)
...
;;; structs stuff
    ...
    (define-struct				system-structs)
    (struct?					system-structs)
    (struct-and-std?				system-structs)
    (make-struct-type				system-structs)
    (struct-type-descriptor?			system-structs)
    (struct-type-name				system-structs)
    (struct-type-symbol				system-structs)
    (struct-type-field-names			system-structs)
    (struct-type-destructor			system-structs)
    (set-struct-type-printer!			system-structs)
    (struct-type-printer			system-structs)
    (set-struct-type-destructor!		system-structs)
    (struct-type-constructor			system-structs)
    (struct-type-predicate			system-structs)
    (struct-type-field-accessor			system-structs)
    (struct-type-field-mutator			system-structs)
    (struct-type-field-method			system-structs)
    (struct-length				system-structs)
    (struct-field-names				system-structs)
    (struct-ref					system-structs)
    (struct-set!				system-structs)
    (struct-and-std-ref				system-structs)
    (struct-and-std-set!			system-structs)
    (struct-printer				system-structs)
    (struct-destructor				system-structs)
    (struct-name				system-structs)
    (struct-std					system-structs)
    (struct=?					system-structs)
    (struct!=?					system-structs)
    (struct-reset!				system-structs)
    (struct-guardian-logger			system-structs)
    (struct-guardian-log			system-structs)
    (struct-hash				system-structs)
...
;;;
    (make-code					system-code-objects)
    (code?					system-code-objects)
    (code-reloc-vector				system-code-objects)
    (code-freevars				system-code-objects)
    (code-size					system-code-objects)
    (code-set!					system-code-objects)
    (set-code-reloc-vector!			system-code-objects)
    (set-code-annotation!			system-code-objects)
    (code-ref					system-code-objects)
    (code->thunk				system-code-objects)
    (code-reloc-vector->sexp			system-code-objects)
    (procedure-annotation			system-code-objects)
    ;;
...
;;;
    ...
    (vector-hash				v $language)
    (list-hash					v $language)
    (pair-hash					v $language)
    (ipair-hash					v $language)
    (fixnum-hash				v $language)
    (bignum-hash				v $language)
    (exact-integer-hash				v $language)
    (flonum-hash				v $language)
    (ratnum-hash				v $language)
    (cflonum-hash				v $language)
    (compnum-hash				v $language)
    ...
    (method					v $language)
    (virtual-method				v $language)
    (seal-method				v $language)
    (private					v $language)
    (protected					v $language)
    (public					v $language)
    (hash					v $language)
    (rcd-rtd					v $language)
...
;;; built-in Scheme object types
    ...
    (<promise>					v $language)
    (<enum-set>					v $language)
    (<enum-set-indexer>				v $language)
    (<enum-set-constructor>			v $language)
    (<utsname>					v $language)
    (<sentinel>					v $language)
    (<stats>					v $language)
...
;;; --------------------------------------------------------------------
    ...
    (<core-type-descriptor>			system-type-descriptors)
    #;(make-core-type-descriptor)
    (core-type-descriptor?			system-type-descriptors)
    (core-type-descriptor.name			system-type-descriptors)
    (core-type-descriptor.parent		system-type-descriptors)
    (core-type-descriptor.type-predicate	system-type-descriptors)
    (core-type-descriptor.equality-predicate	system-type-descriptors)
    (core-type-descriptor.comparison-procedure	system-type-descriptors)
    (core-type-descriptor.hash-function		system-type-descriptors)
    (core-type-descriptor.uid			system-type-descriptors)
    (core-type-descriptor.uids-list		system-type-descriptors)
    (core-type-descriptor.method-retriever	system-type-descriptors)
    (core-type-descriptor.implemented-interfaces system-type-descriptors)
    (core-type-descriptor=?			system-type-descriptors)
    (core-type-descriptor.ancestor-des*		system-type-descriptors)
    (core-type-descriptor.parent-and-child?	system-type-descriptors)
    ...
    (<nebytevector>-ctd				system-type-descriptors)
    (<cflonum>-ctd				system-type-descriptors)
    (<char>-ctd					system-type-descriptors)
    (<code>-ctd					system-type-descriptors)
    (<complex>-ctd				system-type-descriptors)
    (<compnum>-ctd				system-type-descriptors)
    (<compound-condition>-ctd			system-type-descriptors)
    (<condition>-ctd				system-type-descriptors)
    (<empty-string>-ctd				system-type-descriptors)
    (<empty-vector>-ctd				system-type-descriptors)
    (<enum-set>-ctd				system-type-descriptors)
    (<eof>-ctd					system-type-descriptors)
    (<exact-compnum>-ctd			system-type-descriptors)
    (<exact-integer>-ctd			system-type-descriptors)
    (<false>-ctd				system-type-descriptors)
    (<fixnum>-ctd				system-type-descriptors)
    ...
    (<keyword>-ctd				system-type-descriptors)
    (<tcbucket>-ctd				system-type-descriptors)
    (<list>-ctd					system-type-descriptors)
    (<memory-block>-ctd				system-type-descriptors)
    (<negative-bignum>-ctd			system-type-descriptors)
    (<negative-fixnum>-ctd			system-type-descriptors)
    (<negative-flonum>-ctd			system-type-descriptors)
    (<negative-ratnum>-ctd			system-type-descriptors)
    (<negative-zero-flonum>-ctd			system-type-descriptors)
    (<nelist>-ctd				system-type-descriptors)
    (<nestring>-ctd				system-type-descriptors)
    (<nevector>-ctd				system-type-descriptors)
    (<non-zero-cflonum>-ctd			system-type-descriptors)
    ...
    (<positive-bignum>-ctd			system-type-descriptors)
    (<positive-fixnum>-ctd			system-type-descriptors)
    (<positive-flonum>-ctd			system-type-descriptors)
    (<positive-ratnum>-ctd			system-type-descriptors)
    (<positive-zero-flonum>-ctd			system-type-descriptors)
    (<procedure>-ctd				system-type-descriptors)
    (<promise>-ctd				system-type-descriptors)
    (<rational-valued>-ctd			system-type-descriptors)
    (<rational>-ctd				system-type-descriptors)
    (<ratnum>-ctd				system-type-descriptors)
    (<reader-annotation>-ctd			system-type-descriptors)
    (<real-valued>-ctd				system-type-descriptors)
    (<real>-ctd					system-type-descriptors)
    (<record-constructor-descriptor>-ctd	system-type-descriptors)
    (<record-type-descriptor>-ctd		system-type-descriptors)
    (<record>-ctd				system-type-descriptors)
    (<core-type-descriptor>-ctd			system-type-descriptors)
    (<sentinel>-ctd				system-type-descriptors)
    (<stats>-ctd				system-type-descriptors)
    (<string>-ctd				system-type-descriptors)
    ...
    (<true>-ctd					system-type-descriptors)
    (<utsname>-ctd				system-type-descriptors)
    (<vector>-ctd				system-type-descriptors)
    (<void>-ctd					system-type-descriptors)
    (<would-block>-ctd				system-type-descriptors)
    (<zero-cflonum>-ctd				system-type-descriptors)
    (<zero-compnum>-ctd				system-type-descriptors)
    (<zero-fixnum>-ctd				system-type-descriptors)
    (<zero-flonum>-ctd				system-type-descriptors)
...
;;; object type descriptors
    ...
    (<descriptors-signature>					system-type-descriptors)
    (make-descriptors-signature					system-type-descriptors)
    (descriptors-signature?					system-type-descriptors)
    (descriptors-signature.object-type-descrs			system-type-descriptors)
    (descriptors-signature.super-and-sub?			system-type-descriptors)
    (descriptors-signature.match-formals-against-operands	system-type-descriptors)
;;;
    (<lambda-descriptors>-rtd)
    (<lambda-descriptors>-rcd)
    (<lambda-descriptors>					system-type-descriptors)
    (make-lambda-descriptors					system-type-descriptors)
    (lambda-descriptors?					system-type-descriptors)
    (lambda-descriptors.retvals					system-type-descriptors)
    (lambda-descriptors.argvals					system-type-descriptors)
    (lambda-descriptors=?					system-type-descriptors)
    (lambda-descriptors.super-and-sub?				system-type-descriptors)
    (lambda-descriptors.match-formals-against-operands		system-type-descriptors)
;;;
    (<case-lambda-descriptors>-rtd)
    (<case-lambda-descriptors>-rcd)
    (<case-lambda-descriptors>					system-type-descriptors)
    (make-case-lambda-descriptors				system-type-descriptors)
    (case-lambda-descriptors?					system-type-descriptors)
    (case-lambda-descriptors.clause-signature*			system-type-descriptors)
    (case-lambda-descriptors=?					system-type-descriptors)
    (case-lambda-descriptors.super-and-sub?			system-type-descriptors)
    (case-lambda-descriptors.match-formals-against-operands	system-type-descriptors)
;;;
    (<compound-condition-type-descr>-rtd)
    (<compound-condition-type-descr>-rcd)
    (<compound-condition-type-descr>			system-type-descriptors)
    (make-compound-condition-type-descr			system-type-descriptors)
    (compound-condition-type-descr?			system-type-descriptors)
    (compound-condition-type-descr.component-des*	system-type-descriptors)

    (<hashtable-type-descr>-rtd)
    (<hashtable-type-descr>-rcd)
    (<hashtable-type-descr>			system-type-descriptors)
    (make-hashtable-type-descr			system-type-descriptors)
    (hashtable-type-descr?			system-type-descriptors)
    (hashtable-type-descr.key-des		system-type-descriptors)
    (hashtable-type-descr.val-des		system-type-descriptors)

    (<alist-type-descr>-rtd)
    (<alist-type-descr>-rcd)
    (<alist-type-descr>				system-type-descriptors)
    (make-alist-type-descr			system-type-descriptors)
    (alist-type-descr?				system-type-descriptors)
    (alist-type-descr.key-des			system-type-descriptors)
    (alist-type-descr.val-des			system-type-descriptors)
;;;
    (<pair-type-descr>-rtd)
    (<pair-type-descr>-rcd)
    (<pair-type-descr>				system-type-descriptors)
    (make-pair-type-descr			system-type-descriptors)
    (pair-type-descr?				system-type-descriptors)
    (pair-type-descr.car-des			system-type-descriptors)
    (pair-type-descr.cdr-des			system-type-descriptors)

    (<pair-of-type-descr>-rtd)
    (<pair-of-type-descr>-rcd)
    (<pair-of-type-descr>			system-type-descriptors)
    (make-pair-of-type-descr			system-type-descriptors)
    (pair-of-type-descr?			system-type-descriptors)
    (pair-of-type-descr.item-des		system-type-descriptors)

    (<list-type-descr>-rtd)
    (<list-type-descr>-rcd)
    (<list-type-descr>				system-type-descriptors)
    (make-list-type-descr			system-type-descriptors)
    (make-null-or-list-type-descr		system-type-descriptors)
    (list-type-descr?				system-type-descriptors)
    (list-type-descr.item-des*			system-type-descriptors)
    (list-type-descr.length			system-type-descriptors)
    (list-type-descr.car			system-type-descriptors)
    (list-type-descr.cdr			system-type-descriptors)

    (<list-of-type-descr>-rtd)
    (<list-of-type-descr>-rcd)
    (<list-of-type-descr>			system-type-descriptors)
    (make-list-of-type-descr			system-type-descriptors)
    (list-of-type-descr?			system-type-descriptors)
    (list-of-type-descr.item-des		system-type-descriptors)

    (<vector-type-descr>-rtd)
    (<vector-type-descr>-rcd)
    (<vector-type-descr>			system-type-descriptors)
    (make-vector-type-descr			system-type-descriptors)
    (vector-type-descr?				system-type-descriptors)
    (vector-type-descr.item-des*		system-type-descriptors)
    ...
    (make-enumeration-type-descr		system-type-descriptors)
    (enumeration-type-descr?			system-type-descriptors)
    (enumeration-type-descr.symbol*		system-type-descriptors)
    (enumeration-type-descr.length		system-type-descriptors)

    (<closure-type-descr>-rtd)
    (<closure-type-descr>-rcd)
    (<closure-type-descr>			system-type-descriptors)
    (make-closure-type-descr			system-type-descriptors)
    (closure-type-descr?			system-type-descriptors)
    (closure-type-descr.signature		system-type-descriptors)
;;;
    (<ancestor-of-type-descr>-rtd)
    (<ancestor-of-type-descr>-rcd)
    (<ancestor-of-type-descr>			system-type-descriptors)
    (make-ancestor-of-type-descr		system-type-descriptors)
    (ancestor-of-type-descr?			system-type-descriptors)
    (ancestor-of-type-descr.item-des		system-type-descriptors)
    (ancestor-of-type-descr.ancestor-des*	system-type-descriptors)
;;;
    (<interface-type-descr>-rtd)
    (<interface-type-descr>-rcd)
    (<interface-type-descr>			system-type-descriptors)
    (make-interface-type-descr			system-type-descriptors)
    (interface-type-descr?			system-type-descriptors)
    (interface-type-descr.type-name		system-type-descriptors)
    (interface-type-descr.uid			system-type-descriptors)
    (interface-type-descr.parent-type-descriptor system-type-descriptors)
    (interface-type-descr.implemented-interface-uids system-type-descriptors)
    (interface-type-descr.method-prototype-names system-type-descriptors)
    (interface-type-descr.method-retriever	system-type-descriptors)
;;;
    (<union-type-descr>-rtd)
    (<union-type-descr>-rcd)
    (<union-type-descr>				system-type-descriptors)
    (make-union-type-descr			system-type-descriptors)
    (union-type-descr?				system-type-descriptors)
    (union-type-descr.item-des*			system-type-descriptors)

    (<intersection-type-descr>-rtd)
    (<intersection-type-descr>-rcd)
    (<intersection-type-descr>				system-type-descriptors)
    (make-intersection-type-descr			system-type-descriptors)
    (intersection-type-descr?				system-type-descriptors)
    (intersection-type-descr.item-des*			system-type-descriptors)

    (<complement-type-descr>-rtd)
    (<complement-type-descr>-rcd)
    (<complement-type-descr>				system-type-descriptors)
    (make-complement-type-descr				system-type-descriptors)
    (complement-type-descr?				system-type-descriptors)
    (complement-type-descr.item-des			system-type-descriptors)
;;;
    (type-descriptor-of					system-type-descriptors)
    (object-type-descr=?				system-type-descriptors)
    (object-type-descr.parent				system-type-descriptors)
    (object-type-descr.ancestor-des*			system-type-descriptors)
    (object-type-descr.matching-super-and-sub?		system-type-descriptors)
    (object-type-descr.compatible-super-and-sub?	system-type-descriptors)
    (object-type-descr.matching-formal-and-operand	system-type-descriptors)
...
;;; overloaded functions: late binding
    ...
    (<overloaded-function-descriptor>			system-type-descriptors)
    (make-overloaded-function-descriptor		system-type-descriptors)
    (overloaded-function-descriptor?			system-type-descriptors)
    (overloaded-function-descriptor.register!		system-type-descriptors)
    (overloaded-function-descriptor.select-matching-entry system-type-descriptors)
    (overloaded-function-late-binding			system-type-descriptors)

;;; --------------------------------------------------------------------
;;; type descriptors syntaxes

    (type-descriptor=?					system-type-descriptors)
    (type-descriptor-parent				system-type-descriptors)
    (type-descriptor-ancestors				system-type-descriptors)
    (type-descriptor-super-and-sub?			system-type-descriptors)
    (type-descriptor-matching				system-type-descriptors)
    (descriptors-signature-matching			system-type-descriptors)
...
;;; configuration options
    ...
    (print-verbose-messages?				system-options)
    (print-debug-messages?				system-options)
    (print-library-debug-messages?			system-options)
    (print-loaded-libraries?				system-options)
    (debug-mode-enabled?				system-options)
    (drop-assertions?					system-options)
    ...
    (vicare-built-with-ffi-enabled			system-options)
    (vicare-built-with-iconv-enabled			system-options)
    (vicare-built-with-posix-enabled			system-options)
    (vicare-built-with-glibc-enabled			system-options)
    (vicare-built-with-linux-enabled			system-options)
...
;;; POSIX functions
    ...
    (strerror					v $language system-posix)
    (errno->string				system-posix)
    (getenv					v $language system-posix)
    (environ					v $language system-posix)
    (mkdir					system-posix)
    (mkdir/parents				system-posix)
    (real-pathname				system-posix)
    (file-pathname?				system-posix)
    (file-string-pathname?			system-posix)
    (file-bytevector-pathname?			system-posix)
    (file-absolute-pathname?			system-posix)
    (file-relative-pathname?			system-posix)
    (file-string-absolute-pathname?		system-posix)
    (file-string-relative-pathname?		system-posix)
    (file-bytevector-absolute-pathname?		system-posix)
    (file-bytevector-relative-pathname?		system-posix)
    (file-colon-search-path?			system-posix)
    (file-string-colon-search-path?		system-posix)
    (file-bytevector-colon-search-path?		system-posix)
    (list-of-pathnames?				system-posix)
    (list-of-string-pathnames?			system-posix)
    (list-of-bytevector-pathnames?		system-posix)
    (file-modification-time			system-posix)
    (split-pathname-root-and-tail		system-posix)
    (search-file-in-environment-path		system-posix)
    (search-file-in-list-path			system-posix)
    (split-pathname				system-posix)
    (split-pathname-bytevector			system-posix)
    (split-pathname-string			system-posix)
    (split-search-path				system-posix)
    (split-search-path-bytevector		system-posix)
    (split-search-path-string			system-posix)
    (vicare-argv0				v $language system-posix)
    (vicare-argv0-string			v $language system-posix)
...
;;; library infrastructure
    ...
    (current-library-collection				$libraries)
...
;;;
    ...
    (object-type-spec.implemented-interfaces		$expander)
    ...
    (intersection-of-type-specs				$expander)
    (intersection-type-spec?				$expander)
    ...
    (make-pair-of-type-spec				$expander)
    (pair-of-type-spec?					$expander)
...
;;;
    ...
    (&expand-time-type-signature-warning-void-operand		$expander)
    (make-expand-time-type-signature-warning-void-operand	$expander)
    (expand-time-type-signature-warning-void-operand?		$expander)
...
;;;
    ...
    (type-annotation-ancestors				$expander)
    (type-annotation-syntax				$expander)
    (type-annotation-matching				$expander)
    (type-signature-super-and-sub?			$expander)
    (type-signature-common-ancestor			$expander)
    (type-signature-matching				$expander)
    (type-signature-union				$expander)
;;;
...
;;; compiler stuff
    ...
    (current-primitive-locations			$compiler)
    (strict-r6rs-compilation				$compiler)
    ...
    (check-compiler-pass-preconditions			$compiler)
    ...
    (compile-core-expr-to-port				$compiler)
...
;;; (vicare system $hashtables)
    ...
    ($vector-hash				$hashtables)
    ($list-hash					$hashtables)
    ($pair-hash					$hashtables)
    ($ipair-hash				$hashtables)
    ($char-ci-hash				$hashtables)
    ($char-hash					$hashtables)
    ($fixnum-hash				$hashtables)
    ($bignum-hash				$hashtables)
    ($exact-integer-hash			$hashtables)
    ($ratnum-hash				$hashtables)
    ($flonum-hash				$hashtables)
    ($cflonum-hash				$hashtables)
    ($compnum-hash				$hashtables)
	   ...
	   (let next-library-entry ((entries LIBRARY-LEGEND))
	     ...
	     (cond ((null? entries)
		   ...
		   ((entry.required? (car entries))
		    ...
		    (cons (libraries::find-library-by-name (entry.library-name (car entries)))
...
(time-it "the entire bootstrap process"
  (lambda ()
	...
	(time-it "macro expansion"
	  (lambda ()
	    (parameterize ((libraries::current-library-collection bootstrap-collection))
