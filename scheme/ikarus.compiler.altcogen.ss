...
(define (compile-call-frame frame-words-count livemask-vec multiarg-rp call-sequence)
  ...
  ;;
  ;;FRAME-WORDS-COUNT  must be  a non-negative  fixnum representing  the
  ;;number of  words on  the stack frame  already containing  the Scheme
  ;;arguments for the closure to be called.
  ;;
  ;;LIVEMASK-VEC  must  be a  vector  of  fixnums each  representing  an
  ;;octect; the bits in the octets  are live flags for the machine words
  ;;on the stack of the caller.  See the garbage collector for details.
  ;;
  ...
  ;;already on the stack, and  the Frame Pointer Register references the
  ;;return address of the uplevel function.  The situation on the Scheme
  ;;stack is as follows:
  ;;
  ...
  ;;           low memory
  ;;
  ;;  in this picture FRAME-WORDS-COUNT  is 3 describing: uplevel return
  ;;  address,  uplevel Scheme  argument 0,  uplevel Scheme  argument 1.
  ;;   Before executing  the  "call" assembly  instruction:  we need  to
  ;;  adjust the FPR so that  it references the machinw word right above
  ;;  the "empty word":
  ;;
  ;;           high memory
  ;;   |                           |
