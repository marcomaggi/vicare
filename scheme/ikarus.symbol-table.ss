...
;;
;;Field name: length
;;Accessor: symbol-table-length TABLE
;;Mutator: set-symbol-table-length! TABLE
...
(define (extend-table table)
  ...
  (let* ((vec1	(symbol-table-vec table))
    ...
    (when (unsafe.fx< len1 MAX-VECTOR-LENGTH)
...
(define (intern-symbol! sym idx table)
  ...
  ;;
  (let ((number-of-interned-symbols (symbol-table-length table)))
	...
	(let ((n (unsafe.fxadd1 number-of-interned-symbols)))
	  (set-symbol-table-length! table n)
...
(define (unintern sym table)
  ...
  ;;
  (set-symbol-table-length! table (unsafe.fxsub1 (symbol-table-length table)))
...
(module (string->symbol initialize-symbol-table! $symbol-table-size)
  ;;This is  the actual table used  at run-time.  By  experiments it has
  ;;been determined that, after  initialising the table with the symbols
  ;;from the boot  process, the number of buckets  is 4096.  Notice that
  ;;the mask is always one less than the vector size.  (Marco Maggi; Oct
  ;;31, 2011)
  ;;
  (define table
  ...
  (define (initialize-symbol-table!)
    ...
    ;;from it and store them in TABLE.
    ;;
    (define (intern-car x)
      (when (pair? x)
	(let ((sym (unsafe.car x)))
	  (intern-symbol! sym
			  (unsafe.fxand (symbol-hash sym) (symbol-table-mask table))
			  table))
	(intern-car (unsafe.cdr x))))
    (vector-for-each intern-car (foreign-call "ikrt_get_symbol_table"))
    ;; (pretty-print (list 'buckets (vector-length (symbol-table-vec table))
    ;; 			'number-of-interned-symbols ($symbol-table-size)))
    )
