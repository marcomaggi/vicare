...
;;?RAND; a PRIMREF operator is left untouched.  The type descriptions are values of
;;type CORE-TYPE-TAG whose represented types are inferred by this compiler pass.
;;
;;We are interested in adding type annotations only to operands of FUNCALL structs:
;;those are  the only  places where  it does  matter which  type a  returned object
;;has.
;;
...
;;  object.
;;
;;* If the tag "T:vector" is introduced for the operand ?RAND in:
;;
...
;;
;;The input language  expression can contain structs of type  TYPED-EXPR, which are
;;similar to  KNOWN structs:
;;
;;   (typed-expr ?expr ?expr-type)
;;
;;TYPED-EXPR structs may  appear anywhere in the  input source and are  meant to be
;;generated by the expander whenever it is able to determine the type of the values
...
;;an instance o CORE-TYPE-TAG representing the  type informations of the PRELEX.  The
;;entries are kept sorted in the alist:
;;
;;   ((0 . ?prelex-type-0)
;;    (1 . ?prelex-type-1)
;;    (2 . ?prelex-type-2)
...
(define (extend-env prel tag env)
  ;;Add  the given  PRELEX  and the  instance of  CORE-TYPE-TAG  to the  environment.
...
(define (%infer-core-types x env)
  ...
  (define* (V x env)
    ...
    ;;X.
    ;;
    ;;3. An instance of CORE-TYPE-TAG representing  the type of the value returned by
    ;;X.
    ;;
    (struct-case x
      ((constant x.const)
      ...
      ((typed-expr expr declared-core-type)
       ;;The code that  generated the input core language form  was able to determine
       ;;the core  type for  EXPR.  We  validate and use  such declaration  here.  We
       ;;extend the environment if EXPR is a PRELEX struct.
       ;;
       (receive (expr^ env^ inferred-core-type)
	   (V expr env)
	 (case (core-type-tag-is-a? inferred-core-type declared-core-type)
	   ((yes maybe)
	    ;;FIXME Is it fine to always  return CORE-TYPE?  If we can determine that
	    ;;INFERRED-CORE-TYPE  is more  refined than  DECLARED-CORE-TYPE, then  it
	    ;;makes sense to return INFERRED-CORE-TYPE.
	    ;;
	    ...
	    ;;
	    ;;   (and
	    ;;     (eq? 'yes   (core-type-tag-is-a? inferred-core-type declared-core-type))
	    ;;     (eq? 'maybe (core-type-tag-is-a? declared-core-type inferred-core-type)))
	    ;;
	    ;;is    true,   then    INFERRED-CORE-TYPE   is    more   refined    than
	    ;;DECLARED-CORE-TYPE.  (Marco Maggi; Wed Nov 19, 2014)
	    ;;
	    (values expr^
		    (struct-case expr^
		      ((prelex)
		       (extend-env expr^ declared-core-type env^))
		      (else
		       env^))
		    declared-core-type))
	   (else
      ...
      ((seq x.e0 x.e1)
       ...
       ;;
       ;;NOTE The return value of X.E0 is discarded.  E0.TAG.UNUSED is an instance of
       ;;CORE-TYPE-TAG  representing  the  type  of the  X.E0's  return  value;  such
       ;;instance is unused.
       ;;
       ...
       ;;
       ;;NOTE We want to allow the idiom:
       ;;
       ;;   (begin
       ;;     (typed-expr ?prel-0 ?type-0)
       ;;     (typed-expr ?prel-1 ?type-1)
       ;;     ...
       ;;     ?expr)
       ;;
       ;;to  declare the  types  of PRELEX  structs  used in  ?EXPR;  for this  case:
       ;;environment extension is  performed by the TYPED-EXPR  processing.  The code
       ;;after the TYPED-EXPR processing would be:
       ;;
       ;;   (seq
       ;;     ?prel-0
       ;;     (seq
       ...
       ;;       ?expr)
       ;;
       ;;here we want  to make sure that PRELEX structs  in "for side-effect" context
       ;;are removed.
       ;;
...
;;; --------------------------------------------------------------------
  ...
  (define (%determine-prelex-type prel env)
    ...
    ;;PREL; if  found: return an instance  of CORE-TYPE-TAG representing the  type of
    ;;PREL previously determined, otherwise return "T:object".
    ;;
...
;;; --------------------------------------------------------------------
  ...
  (module (inject)
    ...
    (define (%extend* rand* rand.tag* env)
      ...
      ;;instances in RAND.TAG*.  Return the extended environment.
      ;;
...
;;; --------------------------------------------------------------------
  ...
  (module (inject*)
    ...
    (define (%extend* rand* env rand.tag)
      ...
      ;;same CORE-TYPE-TAG instance RAND.TAG.  Return the extended environment.
      ;;
      ;;RAND* can  be a list of  structs representing recordised code,  either PRELEX
      ;;structs, KNOWN  structs or  some other struct  type; if a  RAND is  neither a
      ;;PRELEX nor a KNOWN  holding a PRELEX: it is silently  skipped because no type
