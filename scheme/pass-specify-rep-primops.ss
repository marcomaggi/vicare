...
(section ;;; helpers
 ...
 (define (tag-test x mask tag)
   ;;Test if  X is a word  of type TAG; X  must be a  word referencing a
   ;;Scheme value.   When not  false: use MASK  to extract bits  from X,
...
(section ;;; vectors
 ...
 (define-primop $vector-ref unsafe
   [(V x i)	;if it appears in "expression value" position
    (or
     (struct-case i
		  [(constant i)
		   (and (fx? i)
