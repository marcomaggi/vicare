...
;;; arithmetic operators
  ...
  (check (infix 1 nau.+ 2)		=> (nau.+ 1 2))
  (check (infix 1 nau.+ 2 nau.+ 3)	=> (nau.+ (nau.+ 1 2) 3))
  (check (infix 1 nau.+ 2 nau.- 3)	=> (nau.- (nau.+ 1 2) 3))
  (check (infix 1 nau.+ (2 nau.+ 3))	=> (nau.+ 1 (nau.+ 2 3)))
  (check (infix 1 nau.+ (2 nau.- 3))	=> (nau.+ 1 (nau.- 2 3)))

  (check (infix 1 nau.* 1)		=> (nau.* 1 1))
  (check (infix 1 nau.* 2 nau.* 3)	=> (nau.* (nau.* 1 2) 3))
  (check (infix 1 nau.* 2 nau./ 3)	=> (nau./ (nau.* 1 2) 3))
  (check (infix 1 nau.* (2 nau.* 3))	=> (nau.* 1 (nau.* 2 3)))
  (check (infix 1 nau.* (2 nau./ 3))	=> (nau.* 1 (nau./ 2 3)))
...
;;; bitwise operators
  ...
  (let ((a #b0101) (b #b1111))
    (check (infix a & b)	=> (bitwise-and a b)))

  (let ((a #b0101) (b #b1101))
    (check (infix a ! b)	=> (bitwise-ior a b)))

  (let ((a #b0111) (b #b1101))
    (check (infix a ^ b)	=> (bitwise-xor a b)))

  (let ((a #b0101))
    (check (infix ~ a)		=> (bitwise-not a)))

  (let ((a #b0111) (b 3))
    (check (infix a << b)	=> (bitwise-arithmetic-shift-left a b)))

  (let ((a #b01110000) (b 3))
    (check (infix a >> b)	=> (bitwise-arithmetic-shift-right a b)))
...

(parameterise ((check-test-name 'fixnums))
...

(parameterise ((check-test-name 'flonums))
...

(parameterise ((check-test-name	'precedence))
