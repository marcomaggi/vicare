...
;;;Contents: tests for customisable cond-expand syntax
...

(parametrise ((check-test-name	'customisation))

  (define-cond-expand cond-expand1
    (lambda (id)
      (free-identifier=? id #'display)))

  (define-cond-expand cond-expand2
    (lambda (id)
      (free-identifier=? id #'display))
    (lambda (id)
...

(parametrise ((check-test-name	'helper))
