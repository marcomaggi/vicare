  ...
  #;(doit* (let ((x (read)))
	 ...
	 (bind ((lex.x_0 (funcall (primref read))))
	   (conditional (funcall (primref fixnum?) lex.x_0)
	     ...
	     (conditional (funcall (primref flonum?) lex.x_0)
		 (funcall (primref display)
		   (known lex.x_0 (T:non-false T:real T:nonimmediate T:inexact T:flonum T:number T:object)))
	       (conditional (funcall (primref bignum?) lex.x_0)
		   (funcall (primref display)
		     (known lex.x_0 (T:bignum T:non-false T:nonimmediate T:exact-real T:real T:exact-integer T:exact T:number T:object)))
		 (conditional (funcall (primref ratnum?) lex.x_0)
		   ...
		   (conditional (funcall (primref compnum?) lex.x_0)
		       (funcall (primref display)
			 (known lex.x_0 ())
			 #;(known lex.x_0 (T:compnum T:non-false T:nonimmediate T:number T:object))
			 )
		     (conditional (funcall (primref cflonum?) lex.x_0)
			 (funcall (primref display)
			   (known lex.x_0 (T:cflonum T:non-false T:nonimmediate T:non-real T:inexact T:number T:object)))
		       (conditional (funcall (primref number?) lex.x_0)
			   (funcall (primref display)
			     (known lex.x_0 (T:non-false T:number T:object)))
			 (conditional (funcall (primref complex?) lex.x_0)
			     (funcall (primref display)
			       (known lex.x_0 (T:cflonum T:non-false T:nonimmediate T:non-real T:inexact T:number T:object)))
			   (conditional (funcall (primref real?) lex.x_0)
			       (funcall (primref display)
				 (known lex.x_0 (T:non-false T:real T:number T:object)))
			     (conditional (funcall (primref rational?) lex.x_0)
				 (funcall (primref display)
				   (known lex.x_0 (T:non-false T:number T:object)))
			       (conditional (funcall (primref exact?) lex.x_0)
				   (funcall (primref display)
				     (known lex.x_0 (T:non-false T:real T:exact T:number T:object)))
				 (conditional (funcall (primref inexact?) lex.x_0)
				     (funcall (primref display)
				       (known lex.x_0 (T:non-false T:nonimmediate T:inexact T:number T:object)))
				   (funcall (primref display) lex.x_0)))))))))))))))
  ...
  (doit* (let ((x (read)))
	   (cond ((null? x)
		 ...
		 (else
		  (display x))))
	 (bind ((lex.x_0 (funcall (primref read))))
	   (conditional (funcall (primref null?) lex.x_0)
	       (funcall (primref display)
		 (known lex.x_0 (T:null T:non-false T:proper-list T:immediate T:object)))
	     (conditional (funcall (primref list?) lex.x_0)
		 (funcall (primref display)
		   (known lex.x_0 (T:non-false T:proper-list T:object)))
	       (conditional (funcall (primref pair?) lex.x_0)
		   (funcall (primref display)
		     (known lex.x_0 (T:non-false T:nonimmediate T:pair T:object)))
		 (funcall (primref display) lex.x_0))))))
...
(parametrise ((check-test-name	'conditionals))
  ...
  (doit* (let ((x (read)))
	 ...
	 (bind ((lex.x_0 (funcall (primref read))))
	   (conditional lex.x_0
	       (funcall (primref display)
		 (known lex.x_0 (T:non-false T:object)))
	     (funcall (primref display)
	       (known lex.x_0 (T:false T:boolean T:immediate T:object))))))
  ...
  (doit* (let ((x (read)))
	   (if (not x)
	     ...
	     (display x)))
	 (bind ((lex.x_0 (funcall (primref read))))
	   (conditional (funcall (primref not) lex.x_0)
	       (funcall (primref display)
		 (known lex.x_0 (T:false T:boolean T:immediate T:object)))
	     (funcall (primref display)
	       (known lex.x_0 (T:non-false T:object))))))
...
;;; inference of type after successful type predicate application
  ...
  (doit* (let ((x (read)))
	 ...
	 (bind ((lex.x_0 (funcall (primref read))))
	   (conditional (funcall (primref number?) lex.x_0)
	       (funcall (primref nan?) (known lex.x_0 (T:non-false T:number T:object)))
	     (constant #f))))
  ...
  (doit* (let ((x (read)))
	 ...
	 (bind ((lex.x_0 (funcall (primref read))))
	   (conditional (funcall (primref fixnum?) lex.x_0)
	       (funcall (primref display)
		 (known lex.x_0 (T:fixnum T:non-false T:exact-real T:real T:exact-integer T:exact T:number T:immediate T:object)))
	     (conditional (funcall (primref string?) lex.x_0)
		 (funcall (primref display)
		   (known lex.x_0 (T:string T:non-false T:nonimmediate T:object)))
	       (funcall (primref display)
		 lex.x_0)))))
  ...
  (begin
    ...
    (doit* (let ((f (lambda (y) y))
	   ...
	   (bind ((lex.f_0 (lambda (lex.y_0) lex.y_0))
		  (lex.x_0 (constant (1 . 2))))
	     (funcall (known lex.f_0 (T:procedure T:non-false T:nonimmediate T:object))
	       (funcall (primref cdr)
		 (known lex.x_0 (T:standalone-pair T:non-false T:nonimmediate T:pair T:object))))))
    ...
    (doit* (let ((f (lambda (y) y))
	     ...
	     (f (cdr x)))
	   (bind ((lex.f_0 (lambda (lex.y_0) lex.y_0))
		  (lex.x_0 (funcall (primref read))))
	     (funcall (known lex.f_0 (T:procedure T:non-false T:nonimmediate T:object))
	       (funcall (primref cdr) lex.x_0))))
