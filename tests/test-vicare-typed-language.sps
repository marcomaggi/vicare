...
;;;
  ...
  (check
    ...
    (=> syntax=?)
    #'(a b) #'(<untagged> <untagged>))

  (check
      (typ.parse-list-of-tagged-bindings #'(a b c))
    (=> syntax=?)
    #'(a b c) #'(<untagged> <untagged> <untagged>))
