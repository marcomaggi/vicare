...
(check-display "*** testing SRFI libraries: SRFI 39, parameters\n")
...
(check ;jump in and out of PARAMETERIZE to exercise the dynamic environment
    (let ((outer	#f)
	  (inner	#f)
	  (i		0)
      ...
      (when inner ;jump back into parameterize after jumping out with OUTER
	(inner))

      (parameterize ((alpha 0))
	...
	(if (= 3 i)
	  ...
	  (begin
	    (set! i (+ 1 i))
	    ;;jump out of parameterize
	    (outer)))))
  => '(4 (0 0 0 0)))

(check ;check that the converter is invoked only once when entering PARAMETERIZE
    (let ((outer	#f)
	  (inner	#f)
	  (i		0)
