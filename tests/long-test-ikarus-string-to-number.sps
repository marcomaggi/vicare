...
;;;Abstract
;;;
;;;	Tests from  the file "scheme/tests/string-to-number.ss"  file in
...
(define (generated-tests)
  ...
  (fprintf (current-error-port) "TESTING ~s tests\n" (length complex))
  (for-each
      (lambda (x)
        (test (car x) (cdr x)))
    complex)
...
(check-display "*** testing Ikarus string to number conversion\n")
(run-tests)
(check-report)
